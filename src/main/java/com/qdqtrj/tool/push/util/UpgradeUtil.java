...
import com.qdqtrj.tool.push.bean.VersionSummary;
import com.qdqtrj.tool.push.dao.TWxAccountMapper;
import com.qdqtrj.tool.push.domain.TWxAccount;
import com.qdqtrj.tool.push.ui.Consts;
import com.qdqtrj.tool.push.ui.dialog.UpdateInfoDialog;
...
public class UpgradeUtil {
    ...
    public static void checkUpdate(boolean initCheck) {
        // 当前版本
        String currentVersion = Consts.APP_VERSION;

        // 从github获取最新版本相关信息
        String versionSummaryJsonContent = HttpUtil.get(Consts.CHECK_VERSION_URL);
    ...
    public static void smoothUpgrade() {
        ...
        if (currentVersion.compareTo(beforeVersion) <= 0) {
            ...
            // 然后取两个版本对应的索引
            String versionSummaryJsonContent = FileUtil.readString(Consts.class.getResource("/version_summary.json"), CharsetUtil.UTF_8);
            versionSummaryJsonContent = versionSummaryJsonContent.replace("\n", "");
            VersionSummary versionSummary = JSON.parseObject(versionSummaryJsonContent, VersionSummary.class);
            ...
            for (int i = beforeVersionIndex; i <= currentVersionIndex; i++) {
                ...
                URL sqlFileUrl = Consts.class.getResource(sqlFile);
                if (sqlFileUrl != null) {
